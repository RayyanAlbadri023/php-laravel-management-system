...
<div class="page">
    ...
    <div class="stats">
        ...
        <div class="stat-card">
            <div class="stat-info">
                <div class="num">{{ $stats['departments'] }}</div>
        ...
        <form method="POST" action="{{ route('employees.store') }}">
            ...
            <div class="form-grid">
                ...
                <select name="department">
                    @foreach(['Engineering','HR','Finance','Marketing','Operations'] as $dept)
                        <option value="{{ $dept }}" {{ old('department')==$dept ? 'selected':'' }}>{{ $dept }}</option>
                    @endforeach
                </select>
                <input type="text"   name="position"   placeholder="Position"   value="{{ old('position') }}"   required>
                <input type="number" name="salary"     placeholder="Salary"     value="{{ old('salary') }}"     step="0.01">
            </div>
